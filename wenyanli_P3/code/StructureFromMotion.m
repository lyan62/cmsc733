...
for iImage = 1 : nImages-1
    str = ['matching',num2str(iImage),'.txt'];
    [rgb, mx, my, m] = LoadMatching(str, iImage, nImages);
    RGB = [RGB;rgb];
    Mx = [Mx;mx];
    My = [My;my];
    M = [M;m];
...
for iImage1 = 1 : nImages-1
    for iImage2 = iImage1+1 : nImages
        ...
        subplot(4,3,c);
        hold on;
        str = [num2str(iImage1),'.jpg'];
        C = imread(str);
        l = floor(length(C(1,:))/3);
        image(0, 0, C);
        str = [num2str(iImage2),'.jpg'];
        C = imread(str);
        image(l, 0, C);
...
R
    ...
    for iImage1 = 1 : length(r_idx)-1
        ...
        RGBsize = size(RGB_now,1);
        RGBsizeCell = cell(RGBsize,1);
        for i = 1:RGBsize
            RGBsizeCell{i}= RGB_now(i,1:3)/255;
            plot3(X(i,1),X(i,3),-X(i,2),'.','Color',RGBsizeCell{i});
        end
...
%         plot3(X(:,1),X(:,3),-X(:,2),'r.');
        ...
        title(['After PNP: Img',num2str(iImage1)]);
        
        hold off;
        
        X3D(idx,:) = X;
        RGB_final(idx,1:3) = RGB_now;
        ReconX(idx) = 1;
...
%     [Cset, Rset, X3D] = BundleAdjustment(K, Cr_set, Rr_set, X3D, ReconX, V_bundle, Mx_bundle, My_bundle);
end
figure;
RGBsize = size(RGB,1);
RGBsizeCell = cell(RGBsize,1);
for i = 1:RGBsize
    RGBsizeCell{i}= RGB(i,1:3)/255;
    if (X3D(i,1)~= 0)
        plot3(X3D(i,1),X3D(i,3),-X3D(i,2),'.','Color',RGBsizeCell{i});
    end
%     plot3(X3D(i,1),X3D(i,3),-X3D(i,2),'b.')
    hold on
end
% plot3(X3D(:,1),X3D(:,3),-X3D(:,2),'b.')
xlabel('X')
ylabel('Z')
zlabel('Y')
axis equal
