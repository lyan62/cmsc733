...
%save image:
%z=getframe; 
%imwrite(z.cdata,[imgdir,['anms1_9_',a,'.png']],'png'); 
...
%[pair1_matchedpts1, pair1_matchedpts2] = feature_match(p,1,2,x,y);

%[pair2_matchedpts1, pair2_matchedpts2] = feature_match(p,3,2,x,y);

%[matched1, matched2] = feature_match(p{2},p{3}); % test
for f = 1:length(files)
    [a b]=strtok(files(f).name,'.');
end
...
for np = 1:num_pairs
    ...
    z=getframe; 
    imwrite(z.cdata,[imgdir,['feature_matching1_',a,'.png']],'png'); 
end
